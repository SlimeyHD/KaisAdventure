...
func toggle_cinematic(horizontal: bool, vertical: bool = false) -> void:
	...
	tween.set_trans(Tween.TRANS_EXPO) # Updating the Tween's transition type.
	...
	if (horizontal): # Checking if `horizontal` is true.
		...
		tween.tween_property(bottom_rect, "global_position", Vector2(0, get_viewport_rect().size.y - HORIZONTAL_CUT_SIZE), CINEMATIC_DURATION) # Tweening the bottom rect.
		tween.tween_property(top_rect, "global_position", Vector2(0, -get_viewport_rect().size.y + HORIZONTAL_CUT_SIZE), CINEMATIC_DURATION) # Tweening the top rect.
	else:
		## Tweening the position of the bottom and the top rect.
		tween.tween_property(bottom_rect, "global_position", Vector2(0, get_viewport_rect().size.y + HORIZONTAL_CUT_SIZE), CINEMATIC_DURATION) # Tweening the bottom rect.
		tween.tween_property(top_rect, "global_position", Vector2(0, -get_viewport_rect().size.y - HORIZONTAL_CUT_SIZE), CINEMATIC_DURATION)# Tweening the top rect.
	...
	if (vertical): # Checking if `horizontal` is true.
		## Tweening the position of the bottom and the top rect.
		tween.tween_property(left_rect, "global_position", Vector2(-get_viewport_rect().size.x + VERTICAL_CUT_SIZE, 0), CINEMATIC_DURATION) # Tweening the left rect.
		tween.tween_property(right_rect, "global_position", Vector2(get_viewport_rect().size.x - VERTICAL_CUT_SIZE, 0), CINEMATIC_DURATION) # Tweening the right rect.
	else:
		## Tweening the position of the bottom and the top rect.
		tween.tween_property(left_rect, "global_position", Vector2(-get_viewport_rect().size.x - VERTICAL_CUT_SIZE, 0), CINEMATIC_DURATION) # Tweening the left rect.
		tween.tween_property(right_rect, "global_position", Vector2(get_viewport_rect().size.x + VERTICAL_CUT_SIZE, 0), CINEMATIC_DURATION) # Tweening the right rect.
